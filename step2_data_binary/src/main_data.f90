...
!************************************************************
 do nfile = 1, counter
   ...
   !write( filename, * ) tag
   write( filename, 1111 ) tag
   filename = '../step1_data/data_nnp/forces_'//trim(adjustl(filename))//'.dat'
   ...
   !111 format( A16, A5, I4, A5, I4, A5, I4, A5, I4 )

   !Charge state
   read(1,*) dummyc, charge
   write(*,*) "charge: ", charge
   write(11) charge
   ...
   !volume
   volume = (lattice**3)*(   (x(2)*y(3)-x(3)*y(2))*z(1) &
                           + (x(3)*y(1)-x(1)*y(3))*z(2) &
                           + (x(1)*y(2)-x(2)*y(1))*z(3) )
   write(*,*) "Volume: ", volume
   write(11) volume
   ...
   do imd = 1, md_steps
     ...
     do i = 1, natom
       read(4,*) dummy, dummy, dummy, force(i,1), force(i,2), force(i,3)
     enddo
