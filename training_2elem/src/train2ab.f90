subroutine train2ab( &
  ...
  weight_x, weight_inv_x, nweight_x, hidden_x, nhidden_x, &
  ...
  weight_y, weight_inv_y, nweight_y, hidden_y, nhidden_y, &
  beta, g_e_x, g_f_x, g_e_y, g_f_y, &
...
double precision weight_y( nweight_y ), weight_inv_y( nweight_y )
double precision hidden_x( natom_x, nhidden_x )
double precision hidden_y( natom_y, nhidden_y )
double precision g_e_x( nweight_x ), g_f_x( nweight_x ), energy_x
double precision g_e_y( nweight_y ), g_f_y( nweight_y ), energy_y
double precision energy0, energy, volume
double precision beta, force( natom, 3 ), force0( natom, 3 )
...
!Y-X
 ...
 !------------------------------------------
 energy_x = 0.0d0; energy_y = 0.0d0
 force = 0.0d0
 !X
 if( io_x == 1 )then
 call energy_force_nnp2ab( &
      ...
      natom, natom_x, energy_x, force, charge, volume )
 endif
 !Y
 if( io_y == 1 )then
 call energy_force_nnp2ab( &
      ...
      natom, natom_y, energy_y, force, charge, volume )
 endif

 energy = energy_x + energy_y 


 ! Differenciate Error function with respect to weight
 !-----------------------------------------------------
 g_e_x = 0.0d0; g_e_y = 0.0d0
 g_f_x = 0.0d0; g_f_y = 0.0d0
 !X
 if( io_x == 1 )then
 call f_ef_deriv2ab( &
      g2_x_x,  g2_deriv_x_x,  num_g2_x_x, &
 ...
 call f_ef_deriv2ab( &
      ...
      g5_y_yy, g5_deriv_y_yy, num_g5_y_yy, &
      num_g_y, &
      network_y, nlayer_y, weight_y, weight_inv_y, nweight_y, hidden_y, nhidden_y, &
      beta, g_e_y, g_f_y, &
      natom, natom_y, energy, energy0, force, force0, charge, volume )
 endif


end subroutine train2ab
