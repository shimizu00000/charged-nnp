subroutine energy_force_nnp2ab( &
           ...
           natom, natom_x, energy, force, charge, volume )
implicit none
integer i, j, k, m, di
integer n1
...
double precision energy, atomic_ene_x( natom_x ), volume
...
double precision dummy_ar(natom_x), dummy_ar1( natom_x, network_x(2), network_x(3) )
 ...
 ! charge
 do i = 1, natom_x
   input_x(i, 1 + sum( num_g_x(1:5) ) ) = dble( charge )*100.0d0/volume
 enddo
