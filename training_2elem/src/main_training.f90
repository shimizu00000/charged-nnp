...
integer imd, md_steps, charge
double precision t0, t1, dummy
...
integer,allocatable,dimension(:) :: &
  num_atom, num_atom_a, num_atom_b, num_atom_mpi
...
integer,allocatable,dimension(:) :: charge_all
double precision,allocatable,dimension(:) :: volume_all
double precision seed(2), volume
...
logical           :: lsave(4)
integer           :: isave(44), memory, iprint, weight_counter, itest, max_epoch
double precision  :: dsave(29), factr, pgtol, f_store, rmse_e_thr, rmse_f_thr
...
integer io_weight, io_list, percent_tst
! rc : cutoff radius
! rn : cutoff radius + rn
...
double precision,allocatable,dimension(:,:) :: &
 ...
 if( myrank == 0 )then
   ...
   call read_input_nnp2( &
        ...
        io_weight, io_list, percent_tst, alpha, beta, &
        factr, pgtol, memory, iprint, rmse_e_thr, rmse_f_thr, max_epoch )
 ...
 call mpi_bcast( io_list, 1, mpi_integer, 0, mpi_comm_world, ierr )
 call mpi_barrier( mpi_comm_world, ierr )
...
!************************************************************
 ...
 allocate( charge_all( num_data_total ) )
 allocate( volume_all( num_data_total ) )
 ...
 do nfile = 1, ceiling( dble(counter)/dble(num_mpi) )
   ...
   if( infile <= counter )then
     ...
     do imd = 1, md_steps
       ...
       charge_all( counter_ene ) = charge
       volume_all( counter_ene ) = volume
 ...
 if ( io_list == 0 )then
   ...
   if( myrank == 0 )then
     ...
     write(*,*) " "
1112 format(I10,I3,A3,A4,I2,A4,I2,I4)
 ...
 elseif( io_list == 1 )then
   ...
   if( myrank == 0 )then
     open(unit=98,file="list.dat",action="read")
     read(98,*) dummyc
     read(98,*) dummyi
     read(98,*) dummyc
     do i = 1, num_data_total_mpi
       read(98,*) dummyi, list_mpi(i), dummyc, &
                  dummyc, info_io_mpi((i-1)*2+1), &
                  dummyc, info_io_mpi((i-1)*2+2)
     enddo
     close(98)
   endif ! myrank == 0
   ...
   allocate( list( num_data_total ) )
   call mpi_barrier( mpi_comm_world, ierr )
   call mpi_scatterv( list_mpi, recvcount, displs, mpi_integer, &
                      list, num_data_total, mpi_integer, &
   ...
   call mpi_scatterv( info_io_mpi, recvcount, displs, mpi_integer, &
                      info_io, num_data_total*2, mpi_integer, &
                      0, mpi_comm_world, ierr )
   call mpi_barrier( mpi_comm_world, ierr )
print*,info_io,myrank
...
91 call mpi_barrier( mpi_comm_world, ierr )
   ...
   if( task(1:2) .eq. 'FG' )then
     ...
     do imd = 1, num_data_total
       ...
       charge = charge_all( imd )
       volume = volume_all( imd )
       ...
       if( list( imd ) == 1 )then
         ...
         do i = 1, natom
           dummyi = 3*(i-1)
           force0(i,1) = force0_all( counter_natom + dummyi + 1 ) !3*(i-1) + 1 )
           force0(i,2) = force0_all( counter_natom + dummyi + 2 ) !3*(i-1) + 2 )
           force0(i,3) = force0_all( counter_natom + dummyi + 3 ) !3*(i-1) + 3 )
         enddo
         ...
         call train2ab( io_a, io_b, &
                        ...
                        energy, energy0, force, force0, charge, volume )

         if( io_a == 1 )then
           deallocate( hidden_a, atomic_ene_a )
           g_a =   g_a &
                 + alpha*( g_e_a ) + beta*( g_f_a )
                 !+ alpha*( g_e_a ) + beta*( g_f_a/3.0d0 )
         ...
         do i = 1, natom
           dummy = 1.0d0/dble(3*natom)
           mse_force_trn = mse_force_trn + ( ( force(i,1)-force0(i,1) )**2 )*dummy !/dble(3*natom) )
           mse_force_trn = mse_force_trn + ( ( force(i,2)-force0(i,2) )**2 )*dummy !/dble(3*natom) )
           mse_force_trn = mse_force_trn + ( ( force(i,3)-force0(i,3) )**2 )*dummy !/dble(3*natom) )
         enddo
       ...
       elseif( list( imd ) == 0 )then
         ...
         do i = 1, natom
           dummy = 1.0d0/dble(3*natom)
           mse_force_tst = mse_force_tst + ( ( force(i,1)-force0(i,1) )**2)*dummy !/dble(3*natom) )
           mse_force_tst = mse_force_tst + ( ( force(i,2)-force0(i,2) )**2)*dummy !/dble(3*natom) )
           mse_force_tst = mse_force_tst + ( ( force(i,3)-force0(i,3) )**2)*dummy !/dble(3*natom) )
         enddo
     ...
     if( myrank == 0 )then
       ...
       if( itera == max_epoch )then
         task(1:2) = 'END'
         write(*,*) " "
         write(*,*) "L-BFGS-B STOP BY MAX EPOCH"
         write(*,*) " "
         !go to 101
       endif
       if( dsqrt( mse_energy_trn_mpi ) <= rmse_e_thr )then
         task(1:2) = 'END'
         write(*,*) " "
         write(*,*) "L-BFGS-B STOP BY RMSE(E) THRESHOLD"
         write(*,*) " "
         !go to 101
       endif
       if( dsqrt( mse_force_trn_mpi ) <= rmse_f_thr )then
         task(1:2) = 'END'
         write(*,*) " "
         write(*,*) "L-BFGS-B STOP BY RMSE(F) THRESHOLD"
         write(*,*) " "
         !go to 101
       endif
     ...
     call mpi_bcast( task, 60, mpi_character, 0, mpi_comm_world, ierr )
     call mpi_barrier( mpi_comm_world, ierr ) !not necessary?
 ...
 do imd = 1, num_data_total
   ...
   do i = 1, natom
     dummyi = 3*(i-1)
     force_all( counter_natom + dummyi + 1 ) = force(i,1)
     force_all( counter_natom + dummyi + 2 ) = force(i,2)
     force_all( counter_natom + dummyi + 3 ) = force(i,3)
 ...
 do imd = 1, num_data_total
   ...
   if( list( imd ) == 1 )then
     ...
     do i = 1, natom
       ...
       force0_all( counter_natom + dummyi + 1 ), &
       force0_all( counter_natom + dummyi + 2 ), &
       force0_all( counter_natom + dummyi + 3 ), &
       force_all(  counter_natom + dummyi + 1 ), &
       force_all(  counter_natom + dummyi + 2 ), &
       force_all(  counter_natom + dummyi + 3 ), &
       imd
     enddo
