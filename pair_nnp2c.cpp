...
PairNNP2c::PairNNP2c(LAMMPS *lmp) : Pair(lmp)
{
...
void PairNNP2c::compute(int eflag, int vflag)
{
  int i,j,k,n,ii,jj,kk,inum,jnum;
  ...
  //printf("\n system_charge in compute %15.9f \n",charge);
  double vol = domain->xprd * domain->yprd * domain->zprd;
  //printf("\n volume %15.9f \n",vol);

  int nall = atom->nlocal + atom->nghost;
  memory->create(g2_deriv_a_a,nall,3,g_a[0],"pair:nnp2:g2_deriv_a_a");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_a[0]; j++) {
  ...
  memory->create(g2_deriv_a_b,nall,3,g_a[1],"pair:nnp2:g2_deriv_a_b");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_a[1]; j++) {
  ...
  memory->create(g5_deriv_a_aa,nall,3,g_a[2],"pair:nnp2:g5_deriv_a_aa");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_a[2]; j++) {
  ...
  memory->create(g5_deriv_a_ab,nall,3,g_a[3],"pair:nnp2:g5_deriv_a_ab");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_a[3]; j++) {
  ...
  memory->create(g5_deriv_a_bb,nall,3,g_a[4],"pair:nnp2:g5_deriv_a_bb");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_a[4]; j++) {
  ...
  memory->create(g2_deriv_b_a,nall,3,g_b[0],"pair:nnp2:g2_deriv_b_a");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_b[0]; j++) {
  ...
  memory->create(g2_deriv_b_b,nall,3,g_b[1],"pair:nnp2:g2_deriv_b_b");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_b[1]; j++) {
  ...
  memory->create(g5_deriv_b_aa,nall,3,g_b[2],"pair:nnp2:g5_deriv_b_aa");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_b[2]; j++) {
  ...
  memory->create(g5_deriv_b_ab,nall,3,g_b[3],"pair:nnp2:g5_deriv_b_ab");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_b[3]; j++) {
  ...
  memory->create(g5_deriv_b_bb,nall,3,g_b[4],"pair:nnp2:g5_deriv_b_bb");
  for (i = 0; i < nall; i++) {
      for (j = 0; j < g_b[4]; j++) {
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < numshort; jj++) {
      ...
      for (kk = 0; kk < numshort; kk++) {
        ...
        k = neighshort[kk];
        ktype = map[type[k]];
        delr2[0] = x[k][0] - xtmp; delr2[1] = x[k][1] - ytmp; delr2[2] = x[k][2] - ztmp;
    ...
    for (j = 0; j < 3; j++) {
      ...
      for (k = 0; k < g_a[2]; k++) g5_deriv_a_aa[i][j][k] = 0.0;
      for (k = 0; k < g_a[3]; k++) g5_deriv_a_ab[i][j][k] = 0.0;
      for (k = 0; k < g_a[4]; k++) g5_deriv_a_bb[i][j][k] = 0.0;
      for (k = 0; k < g_b[0]; k++) g2_deriv_b_a[i][j][k] = 0.0;
      for (k = 0; k < g_b[1]; k++) g2_deriv_b_b[i][j][k] = 0.0;
      for (k = 0; k < g_b[2]; k++) g5_deriv_b_aa[i][j][k] = 0.0;
      for (k = 0; k < g_b[3]; k++) g5_deriv_b_ab[i][j][k] = 0.0;
      for (k = 0; k < g_b[4]; k++) g5_deriv_b_bb[i][j][k] = 0.0;
    }}
...
void PairNNP2c::settings(int narg, char **/*arg*/)
{
...
void PairNNP2c::coeff(int narg, char **arg)
{
...
double PairNNP2c::init_one(int i, int j)
{
...
void PairNNP2c::read_file(char *file)
{
  ...
  while (1) {
    ...
    while (mojiretsu) {
      if (strcmp(mojiretsu,"cutoff_distance") == 0) {
      ...
      else if (strcmp(mojiretsu,"system_charge") == 0) {
        mojiretsu = strtok(NULL," \t\n\r\f");
        if (mojiretsu == NULL) return;
        charge = atof(mojiretsu);
      }
        ...
        char **suji = new char*[ 4*g_a[2]+1 ];
        nsuji = 0;
        suji[nsuji++] = strtok(sen2," \t\n\r\f");
        while ((suji[nsuji++] = strtok(NULL," \t\n\r\f"))) continue;
        eta5_a_aa    = new double[ g_a[2] ];
        theta5_a_aa  = new double[ g_a[2] ];
        zeta5_a_aa   = new double[ g_a[2] ];
        lambda5_a_aa = new double[ g_a[2] ];
        ...
        suji[nsuji++] = strtok(sen2," \t\n\r\f");
        while ((suji[nsuji++] = strtok(NULL," \t\n\r\f"))) continue;
        eta5_a_ab    = new double[ g_a[3] ];
        theta5_a_ab  = new double[ g_a[3] ];
        zeta5_a_ab   = new double[ g_a[3] ];
        lambda5_a_ab = new double[ g_a[3] ];
        ...
        suji[nsuji++] = strtok(sen2," \t\n\r\f");
        while ((suji[nsuji++] = strtok(NULL," \t\n\r\f"))) continue;
        eta5_a_bb    = new double[ g_a[4] ];
        theta5_a_bb  = new double[ g_a[4] ];
        zeta5_a_bb   = new double[ g_a[4] ];
        lambda5_a_bb = new double[ g_a[4] ];
        ...
        char **suji = new char*[ 4*g_b[2]+1 ];
        nsuji = 0;
        suji[nsuji++] = strtok(sen2," \t\n\r\f");
        while ((suji[nsuji++] = strtok(NULL," \t\n\r\f"))) continue;
        eta5_b_aa    = new double[ g_b[2] ];
        theta5_b_aa  = new double[ g_b[2] ];
        zeta5_b_aa   = new double[ g_b[2] ];
        lambda5_b_aa = new double[ g_b[2] ];
        ...
        suji[nsuji++] = strtok(sen2," \t\n\r\f");
        while ((suji[nsuji++] = strtok(NULL," \t\n\r\f"))) continue;
        eta5_b_ab    = new double[ g_b[3] ];
        theta5_b_ab  = new double[ g_b[3] ];
        zeta5_b_ab   = new double[ g_b[3] ];
        lambda5_b_ab = new double[ g_b[3] ];
        ...
        suji[nsuji++] = strtok(sen2," \t\n\r\f");
        while ((suji[nsuji++] = strtok(NULL," \t\n\r\f"))) continue;
        eta5_b_bb    = new double[ g_b[4] ];
        theta5_b_bb  = new double[ g_b[4] ];
        zeta5_b_bb   = new double[ g_b[4] ];
        lambda5_b_bb = new double[ g_b[4] ];
...
/* ---------------------------------------------------------------------- */
void PairNNP2c::sf_g2(int i, int j, double rij,
...
double PairNNP2c::fc(double rij)
{
...
double PairNNP2c::fc_deriv(double rij, double xi, double xj)
{
...
void PairNNP2c::sf_g5(int i, int j, int k, double rij, double rik,
                     double x1, double y1, double z1,
                     double x2, double y2, double z2,
...
/* ---------------------------------------------------------------------- */
void PairNNP2c::hdnnp_f_a(double ***(&g2_deriv_a_a),  int num_g2_a_a,
                          ...
                          int ii, double *(&hidden_a), double &fx, double &fy, double &fz )
{
  ...
  if (nlayer_a == 4) {
    ...
    for (int l0 = 1; l0 <= layer_a[0]-1; l0++){
...
/* ---------------------------------------------------------------------- */
void PairNNP2c::hdnnp_e_b(double *(&g2_b_a),  int num_g2_b_a,
...
/* ---------------------------------------------------------------------- */
void PairNNP2c::hdnnp_f_b(double ***(&g2_deriv_b_a),  int num_g2_b_a,
                          ...
                          int ii, double *(&hidden_b), double &fx, double &fy, double &fz )
{
  ...
  if (nlayer_b == 4) {
    ...
    for (int l0 = 1; l0 <= layer_b[0]-1; l0++){
...
void PairNNP2c::arraynnp()
{
  int n;

  memory->create(hidden_a,nhidden_a,"pair:nnp2:hidden_a");
  for (n = 0; n < nhidden_a; n++) hidden_a[n] = 0.0;
  memory->create(hidden_b,nhidden_b,"pair:nnp2:hidden_b");
  for (n = 0; n < nhidden_b; n++) hidden_b[n] = 0.0;

  // input
  memory->create(input_a,layer_a[0]+1,"pair:nnp2:input_a");
  memory->create(input_b,layer_b[0]+1,"pair:nnp2:input_b");

  // dGdx, dGdy, dGdz
  memory->create(dGdx_a,layer_a[0],"pair:nnp2:dGdx_a");
  memory->create(dGdy_a,layer_a[0],"pair:nnp2:dGdy_a");
  memory->create(dGdz_a,layer_a[0],"pair:nnp2:dGdz_a");

  memory->create(dGdx_b,layer_b[0],"pair:nnp2:dGdx_b");
  memory->create(dGdy_b,layer_b[0],"pair:nnp2:dGdy_b");
  memory->create(dGdz_b,layer_b[0],"pair:nnp2:dGdz_b");

  memory->create(g2_a_a,g_a[0],"pair:nnp2:g2_a_a");
  for (n = 0; n < g_a[0]; n++) g2_a_a[n] = 0.0;
  memory->create(g2_a_b,g_a[1],"pair:nnp2:g2_a_b");
  for (n = 0; n < g_a[1]; n++) g2_a_b[n] = 0.0;
  memory->create(g5_a_aa,g_a[2],"pair:nnp2:g5_a_aa");
  for (n = 0; n < g_a[2]; n++) g5_a_aa[n] = 0.0;
  memory->create(g5_a_ab,g_a[3],"pair:nnp2:g5_a_ab");
  for (n = 0; n < g_a[3]; n++) g5_a_ab[n] = 0.0;
  memory->create(g5_a_bb,g_a[4],"pair:nnp2:g5_a_bb");
  for (n = 0; n < g_a[4]; n++) g5_a_bb[n] = 0.0;

  memory->create(g2_b_a,g_b[0],"pair:nnp2:g2_b_a");
  for (n = 0; n < g_b[0]; n++) g2_b_a[n] = 0.0;
  memory->create(g2_b_b,g_b[1],"pair:nnp2:g2_b_b");
  for (n = 0; n < g_b[1]; n++) g2_b_b[n] = 0.0;
  memory->create(g5_b_aa,g_b[2],"pair:nnp2:g5_b_aa");
  for (n = 0; n < g_b[2]; n++) g5_b_aa[n] = 0.0;
  memory->create(g5_b_ab,g_b[3],"pair:nnp2:g5_b_ab");
  for (n = 0; n < g_b[3]; n++) g5_b_ab[n] = 0.0;
  memory->create(g5_b_bb,g_b[4],"pair:nnp2:g5_b_bb");
  for (n = 0; n < g_b[4]; n++) g5_b_bb[n] = 0.0;
