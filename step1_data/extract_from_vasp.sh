...
char=$( head -1 "char" | awk '(NR==1){print $1}' )
echo $dir $char
echo " "
...
# FORCE FROM OUTCAR
  ...
  echo "--" \
  >  ./data_nnp/forces_$tag\.dat
  natom1=$(( $natom + 1 ))
  grep -A$natom1 "TOTAL-FORCE" ./data_vasp/$dir/OUTCAR_$tag \
  >> ./data_nnp/forces_$tag\.dat
...
# Extract NUMBER OF IONS
  ...
  #echo "charge 3" >> ./data_nnp/info_$tag\.dat
  #echo "charge 3" 
  echo "charge" $char >> ./data_nnp/info_$tag\.dat
  echo "charge" $char
  echo " " >> ./data_nnp/info_$tag\.dat
